...
def main():
    ...
    if args.start_epoch > 0:
        ...
        checkpoint = torch.load("%s/net_params_%d_%d.pth" % (pre_model_dir, args.start_epoch, args.layer_num))
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        if epoch > 90 and epoch % 5 == 0:
            checkpoint = {
                'epoch': epoch,
                'net': model.state_dict(),
                'optimizer': optimizer.state_dict(),
            }
            torch.save(checkpoint, "%s/net_params_%d_%d.pth" % (model_dir, epoch, args.layer_num))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--sensing_rate', type=float, default=0.0100000, help='set sensing rate')
